...
// C-logic + A-risk params: ставим limit-ы на rangeEdge как C, но c риском как у A
// (10 max conc, 10% target margin). Депозит $320 остаётся как у C для прямой
// сравнимости с базовым C-live прогоном.
const VARIANT_C = {
  name: 'C-like-A' as const,
  startingDeposit: 320,
  maxConcurrent: 10,
  targetMarginPct: 10,
}
...
async function main() {
  console.log('Daily Breakout — Variant C LIVE simulator [C-logic + A-risk params]')
  ...
  const outFile = path.join(outDir, `dailybreak_C_likeA_${Date.now()}.json`)
