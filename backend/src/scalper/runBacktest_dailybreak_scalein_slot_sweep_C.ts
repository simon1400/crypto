 ...
 *
 * Variant C: $320 / 20 conc / 5% target margin / 2% risk / LIMIT entry on rangeEdge (maker fee)
 *
 * Run: cd backend && npx tsx src/scalper/runBacktest_dailybreak_scalein_up_C.ts
 */
...
const RISK_PCT = 2
// Slot policy sweep для C при оптимальном combo (33% trigger, 100% size — лучший из combo_C)
const SCALE_IN_FRAC = 1.0  // C optimal — 100% размер от primary
const TRIGGER_PCT = 0.33   // C optimal — 33% пути к TP1

type SlotPolicy = 'shared' | 'freeScaleIn' | 'doubled'
const SLOT_POLICIES: SlotPolicy[] = ['shared', 'freeScaleIn', 'doubled']
let CURRENT_SLOT_POLICY: SlotPolicy = 'shared'
...
// C variant: limit entry on rangeEdge (maker fee, no slip). Не taker.
const C_LIMIT_ENTRY = true
  ...
  for (const [, { candles, startIdx }] of byDay) {
    ...
    for (let i = cfg.rangeBars; i < candles.length && !triggered; i++) {
      ...
      if (c.high > rangeHigh && c.close > rangeHigh) {
        side = 'BUY'
        // C variant: limit entry на rangeHigh (как pre-placed limit, который сработал когда
        // бар пробил уровень). taker = c.close, limit = rangeHigh.
        entryPrice = C_LIMIT_ENTRY ? rangeHigh : c.close
        anchor = rangeHigh
      } else if (c.low < rangeLow && c.close < rangeLow) {
        side = 'SELL'
        entryPrice = C_LIMIT_ENTRY ? rangeLow : c.close
        anchor = rangeLow
      }
      if (!side) continue
...
function findScaleInTrigger(
  ...
  const triggerPrice = entryPrice + TRIGGER_PCT * (tp1 - entryPrice)
  for (let i = primaryEntryIdx + 1; i < m5.length; i++) {
    ...
    const triggered = isLong ? c.close >= triggerPrice : c.close <= triggerPrice
    if (triggered) {
      return { idx: i, price: triggerPrice }
    }
  }
  return null
}
...
function simulate(allTrades: PortfolioTrade[], mode: Mode, btc: BtcRegime): SimResult {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    // Slot policy check
    const cap = VARIANT_A.maxConcurrent
    let slotsFull = false
    if (CURRENT_SLOT_POLICY === 'shared') {
      slotsFull = active.length >= cap
    } else if (CURRENT_SLOT_POLICY === 'freeScaleIn') {
      if (!pt.isScaleIn) {
        const primaryCount = active.filter(a => !a.pt.isScaleIn).length
        slotsFull = primaryCount >= cap
      }
    } else { // doubled
      if (pt.isScaleIn) {
        const scaleInCount = active.filter(a => a.pt.isScaleIn).length
        slotsFull = scaleInCount >= cap
      } else {
        const primaryCount = active.filter(a => !a.pt.isScaleIn).length
        slotsFull = primaryCount >= cap
      }
    }
    if (slotsFull) { skippedConcurrent++; continue }

    const isLong = pt.side === 'BUY'
    // C: limit entry — exact fill на pt.entryPrice (rangeEdge для primary, midTP1 для scale-in), no slip
    const effectiveEntry = C_LIMIT_ENTRY
      ? pt.entryPrice
    ...
    // C: maker fee on limit entry; A/B (taker): taker fee
    const entryFeeRate = C_LIMIT_ENTRY ? MAKER_FEE : TAKER_FEE
...
async function main() {
  console.log('Daily Breakout — Variant C (limit entry): SLOT POLICY sweep (33%/100% scale-in)')
  ...
  console.log(`Variant C: $320 / 20 conc / 5% margin / 2% risk / LIMIT entry on rangeEdge`)
  ...
  // Primary pool — одинаковый для всех combo (зависит только от cfg).
  type PrimaryCache = { trades: PortfolioTrade[]; primarySigs: PrimarySignal[]; m5: OHLCV[]; from: number; to: number; sym: string }
  const primaryCacheByPeriod: Record<'full' | 'train' | 'test', PrimaryCache[]> = {
    full: [], train: [], test: [],
  }
  for (const [sym, m5] of m5BySymbol.entries()) {
    for (const [periodKey, from, to] of [
      ...
      const portfolioTrades = trades.map(t => toPortfolioTrade(sym, t, false))
      primaryCacheByPeriod[periodKey].push({ trades: portfolioTrades, primarySigs, m5, from, to, sym })
    }
  }
  const primaryPools = {
    full: primaryCacheByPeriod.full.flatMap(c => c.trades),
    train: primaryCacheByPeriod.train.flatMap(c => c.trades),
    test: primaryCacheByPeriod.test.flatMap(c => c.trades),
  }
  console.log(`Primary pool: FULL ${primaryPools.full.length} | TRAIN ${primaryPools.train.length} | TEST ${primaryPools.test.length}`)
  console.log()

  // Baseline validation
  const validate = simulate(primaryPools.full, 'baseline', btc)
  console.log(`Baseline check (must be ≈$7881-$8175 for C limit): final=$${validate.finalDeposit.toFixed(2)}`)
  console.log()

  // Pre-compute scale-in pool ОДИН раз (TRIGGER_PCT / SCALE_IN_FRAC фиксированы).
  // Scale-in entry price зависит только от TRIGGER_PCT, size — только в simulate.
  const scaleInPools: Record<'full' | 'train' | 'test', PortfolioTrade[]> = { full: [], train: [], test: [] }
  for (const periodKey of ['full', 'train', 'test'] as const) {
    for (const { primarySigs, m5, from, to, sym } of primaryCacheByPeriod[periodKey]) {
      const scaleInTrades = generateScaleInTrades(m5, primarySigs)
      scaleInTrades
        .filter(t => t.entryTime >= from && t.entryTime <= to)
        .forEach(t => scaleInPools[periodKey].push(toPortfolioTrade(sym, t, true)))
    }
  }
  console.log(`Scale-in pool: FULL ${scaleInPools.full.length} | TRAIN ${scaleInPools.train.length} | TEST ${scaleInPools.test.length}`)
  console.log()

  // Slot policy sweep
  interface Row { policy: SlotPolicy; period: 'FULL' | 'TRAIN' | 'TEST'; baseline: SimResult; scaleIn: SimResult }
  const rows: Row[] = []
  for (const policy of SLOT_POLICIES) {
    CURRENT_SLOT_POLICY = policy
    console.log(`================== SLOT POLICY: ${policy} ==================`)
    for (const [periodName, periodKey] of [['FULL', 'full'], ['TRAIN', 'train'], ['TEST', 'test']] as const) {
      const baselineRes = simulate(primaryPools[periodKey], 'baseline', btc)
      const allTrades = [...primaryPools[periodKey], ...scaleInPools[periodKey]]
      const scaleInRes = simulate(allTrades, 'scaleInUp', btc)
      rows.push({ policy, period: periodName, baseline: baselineRes, scaleIn: scaleInRes })
      ...
      const swr = scaleInRes.scaleInCount > 0 ? (scaleInRes.scaleInWins / scaleInRes.scaleInCount) * 100 : 0
      console.log(
        `${periodName.padEnd(6)} | baseline $${baselineRes.finalDeposit.toFixed(0).padStart(6)} (${retB >= 0 ? '+' : ''}${retB.toFixed(0)}%) | ` +
        `scaleIn $${scaleInRes.finalDeposit.toFixed(0).padStart(8)} (${retS >= 0 ? '+' : ''}${retS.toFixed(0)}%) | ` +
        `Δret=${(retS - retB) >= 0 ? '+' : ''}${(retS - retB).toFixed(0)}pp | ` +
        `R/tr=${fmtR(scaleInRes.rPerTr)} | WR=${scaleInRes.winRate.toFixed(0)}% | DD=${scaleInRes.maxDD.toFixed(0)}% | ` +
        `scaleIn trades=${scaleInRes.scaleInCount} WR=${swr.toFixed(0)}% net=${fmtUsd(scaleInRes.scaleInNet)} | ` +
        `opened=${scaleInRes.opened} (vs ${baselineRes.opened})`
      )
    }
    console.log()
  }

  console.log('================== Summary table (FULL period) ==================')
  console.log('policy        | scaleIn final | ret%    | R/tr  | WR  | DD  | opened | scaleIn trades | scaleIn WR | net$')
  for (const r of rows.filter(r => r.period === 'FULL')) {
    const retS = ((r.scaleIn.finalDeposit / r.scaleIn.startingDeposit - 1) * 100)
    const swr = r.scaleIn.scaleInCount > 0 ? (r.scaleIn.scaleInWins / r.scaleIn.scaleInCount) * 100 : 0
    console.log(
      `${r.policy.padEnd(13)} | $${r.scaleIn.finalDeposit.toFixed(0).padStart(10)} | ${(retS >= 0 ? '+' : '')}${retS.toFixed(0).padStart(6)}% | ${fmtR(r.scaleIn.rPerTr)} | ${r.scaleIn.winRate.toFixed(0)}% | ${r.scaleIn.maxDD.toFixed(0)}% | ${r.scaleIn.opened.toString().padStart(5)} | ${r.scaleIn.scaleInCount.toString().padStart(5)} | ${swr.toFixed(0)}% | ${fmtUsd(r.scaleIn.scaleInNet)}`
    )
  }
  console.log()
