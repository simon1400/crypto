/**
 * Daily Breakout — Variant A backtest comparing OLD logic (no dedup guards)
 * vs NEW logic (carry-over + same-day per-symbol guards).
 *
 * Built on top of the same engine as runBacktest_dailybreak_compare.ts (same
 * generateBreakoutSignals, ladderBacktester, margin guard) — only the
 * portfolio-level guards differ. This way numbers are directly comparable
 * with the existing prod baseline backtests.
 *
 * Config A (prod):
 *   - 23 prod symbols, $500 deposit, 2% risk, 10 max concurrent, 10% target margin
 *   - 0.08% RT fees, 0.10% slippage per side
 *   - BTC ADX(14) on 1h > 20 filter (skip signals when BTC sideways)
 *
 * NOTE: The base engine here uses entry = rangeEdge (idealized fill — no entry
 * slip past the edge). The live engine (dailyBreakoutEngine.ts) uses entry =
 * trigger candle close, which is realistic but harder to model retrospectively.
 * For a portfolio-level dedup comparison the entry-fill detail is symmetric
 * across NEW and OLD scenarios, so the *delta* is meaningful even if absolute
 * cumulative numbers are slightly more optimistic than live.
 *
 * Run: cd backend && npx tsx src/scalper/runBacktest_dailybreak_dedup.ts
 */
...
const SLIPPAGE = 0.0010 // 0.10% per side — Bybit taker realistic
...
async function buildBtcRegime(): Promise<BtcRegime> {
  ...
  return {
    isTrending(t: number) {
      ...
      return v == null ? true : v > BTC_ADX_THRESHOLD  // fail-open
    },
  }
}

// ============================================================================
// Signal generation (same as compare.ts — one breakout per UTC day per symbol)
// ============================================================================
...
// ============================================================================
// Portfolio simulator with margin guard, concurrent cap, ADX, and dedup
// ============================================================================
...
function simulate(
  ...
  let wins = 0, totalR = 0
  const takenSet = new Set<string>()  // key = symbol|utcDate
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    // BTC ADX filter
    if (scenario.applyBtcAdx && !btc.isTrending(pt.entryTime)) {
      skippedBtcAdx++
      continue
    }
    // Carry-over guard
    if (scenario.carryOverGuard) {
      const blocked = active.some(a =>
        a.pt.symbol === pt.symbol && a.pt.utcDate !== pt.utcDate
      )
...
function printMonthlyTable(label: string, r: SimResult) {
  console.log(`--- ${label}: monthly P&L (${r.scenario}) ---`)
...
async function main() {
  console.log('Daily Breakout — Variant A: NEW dedup logic vs OLD (no guards)')
  console.log(`Universe: ${PROD_SYMBOLS.length} symbols | $${STARTING_DEPOSIT} deposit | ${RISK_PCT}% risk | ${MAX_CONCURRENT} max concurrent | ${TARGET_MARGIN_PCT}% target margin`)
  console.log(`Slippage ${(SLIPPAGE * 100).toFixed(2)}%/side | Fees ${(FEES_RT * 100).toFixed(2)}% RT | BTC ADX(14)>${BTC_ADX_THRESHOLD} regime filter`)
  ...
  const NEW_LOGIC: ScenarioFlags = {
    name: '(a) NEW logic (BTC ADX + carry-over + same-day guard)',
    applyBtcAdx: true, carryOverGuard: true, sameDayPerSymbolGuard: true,
  }
  const OLD_LOGIC: ScenarioFlags = {
    name: '(b) OLD logic (BTC ADX only — no dedup)',
    applyBtcAdx: true, carryOverGuard: false, sameDayPerSymbolGuard: false,
  }

  console.log('================== FULL (365d) ==================')
  const fullA = simulate(allFull, NEW_LOGIC, btc, STARTING_DEPOSIT)
  const fullB = simulate(allFull, OLD_LOGIC, btc, STARTING_DEPOSIT)
  printResult('FULL', fullA); printResult('FULL', fullB); console.log()

  console.log('================== TRAIN (60%, ~219d) ==================')
  const trainA = simulate(allTrain, NEW_LOGIC, btc, STARTING_DEPOSIT)
  const trainB = simulate(allTrain, OLD_LOGIC, btc, STARTING_DEPOSIT)
  printResult('TRAIN', trainA); printResult('TRAIN', trainB); console.log()

  console.log('================== TEST (40%, ~146d) ==================')
  const testA = simulate(allTest, NEW_LOGIC, btc, STARTING_DEPOSIT)
  const testB = simulate(allTest, OLD_LOGIC, btc, STARTING_DEPOSIT)
  printResult('TEST', testA); printResult('TEST', testB); console.log()

  console.log('================== Summary: NEW vs OLD ==================')
  const diff = (a: SimResult, b: SimResult, label: string) => {
    const dN = a.opened - b.opened
    const dPnl = a.finalDeposit - b.finalDeposit
    const dPnlPct = ((a.finalDeposit / STARTING_DEPOSIT - 1) - (b.finalDeposit / STARTING_DEPOSIT - 1)) * 100
    console.log(`${label.padEnd(6)} | NEW opened=${a.opened.toString().padStart(4)} R/tr=${fmtR(a.rPerTr)} final=$${a.finalDeposit.toFixed(0).padStart(6)} DD=${a.maxDD.toFixed(1)}%  ||  OLD opened=${b.opened.toString().padStart(4)} R/tr=${fmtR(b.rPerTr)} final=$${b.finalDeposit.toFixed(0).padStart(6)} DD=${b.maxDD.toFixed(1)}%  ||  Δtrades=${dN >= 0 ? '+' : ''}${dN}  Δ$=${fmtUsd(dPnl)}  (${dPnlPct >= 0 ? '+' : ''}${dPnlPct.toFixed(1)}pp)`)
  }
  diff(fullA, fullB, 'FULL')
  diff(trainA, trainB, 'TRAIN')
  diff(testA, testB, 'TEST')
  console.log()

  printMonthlyTable('FULL NEW', fullA)
  console.log()
