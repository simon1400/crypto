/**
 * Daily Breakout — Variant B with EARLY-EXIT rule.
 *
 * Rule: close 100% of position by market as soon as unrealized PnL
 *       reaches currentDepositAtOpen * 2%.
 *       (No $10 minimum — pure 2% of deposit at the moment the trade opened.)
 *
 * Compares against baseline B (no early-exit) using identical:
 *   - Universe: 23 PROD symbols
 *   - Period: 365d (FULL / TRAIN 60% / TEST 40%)
 *   - Sizing: $320 start, 20 max conc, 5% target margin, 2% risk
 *   - Filters: BTC ADX>20, dedup carry-over + same-day, EOD-NO-TP1
 *   - Fees: Binance taker 0.05% / maker 0.02% / slip 0.03% (taker market entry)
 *
 * The early-exit forces a taker market exit (taker fee + slip) on the m5 close
 * that first crossed the threshold. SL/TP precedence within the same bar:
 *   1) SL wick check first (worst case)
 *   2) Early-exit check on bar close (only fires if SL didn't hit)
 *   3) Remaining TP fills processed normally
 *
 * Run: cd backend && npx tsx src/scalper/runBacktest_dailybreak_B_early_exit.ts
 */
...
const EARLY_EXIT_PCT_OF_DEPOSIT = 0.02
...
interface RawSignal {
  ...
  side: 'BUY' | 'SELL'
  entryTime: number
  entryPrice: number   // c.close (live trigger)
  rangeEdge: number    // anchor for TP geometry
  sl: number
  tpLadder: number[]
  utcDate: string
}
...
// ============================================================================
// Per-trade walk-forward simulator over m5 candles.
// Returns list of (time, exitPrice, frac, kind) fills for one trade.
// ============================================================================

type FillKind = 'TP1' | 'TP2' | 'TP3' | 'SL' | 'EOD' | 'EARLY'

interface TradeFill {
  time: number
  price: number
  frac: number      // fraction of original position closed in this fill
  kind: FillKind
}
...
interface SimulatedTrade {
  ...
  side: 'BUY' | 'SELL'
  entryTime: number
  entryPrice: number   // raw signal entry (taker pre-slip)
  sl: number           // initial SL
  tpLadder: number[]
  /** m5 candles for walking the trade (from entry forward, sliced before storing) */
  m5: OHLCV[]
  startIdx: number     // index of entry candle within m5
}

/**
 * Walk one trade forward over m5 candles, applying ladder logic + optional early-exit.
 *
 * earlyExitUsdThreshold: if defined and positive, close 100% by market once
 *   running unrealized PnL on the open remainder (using bar close) reaches the threshold.
 *   The threshold is computed once at trade open (deposit * 2%) and stays constant
 *   for the life of this trade.
 *
 * positionUnits/effectiveEntry: needed to translate price moves into USD PnL.
 * (Computed by caller after sizing.)
 */
function walkTradeFills(
  t: SimulatedTrade,
  effectiveEntryPrice: number,
  positionUnits: number,
  earlyExitUsdThreshold: number | null,
  ...
  // Start walking from the bar AFTER entry candle (consistent with ladderBacktester
  // which checks bar i for triggers and opens a position; subsequent bars manage it).
  for (let i = t.startIdx + 1; i <= lastIdx; i++) {
    const c = t.m5[i]

    // 1) SL wick check (worst case first)
    ...
    // 2) TP wick checks — same logic as ladderBacktester (wick mode, fill on reaction
    //    or on last TP). For early-exit purposes we still want TPs to fire on bars
    //    where they hit, BEFORE checking early-exit on the close of the same bar.
    let tpProgressed = true
    while (tpProgressed && nextTpIdx < t.tpLadder.length && remaining > 1e-9) {
      tpProgressed = false
      const tp = t.tpLadder[nextTpIdx]
      const wickReached = isLong ? c.high >= tp : c.low <= tp
      if (!wickReached) break
      const isLastTp = nextTpIdx === t.tpLadder.length - 1
      ...
      if (fill) {
        ...
        if (actualFrac > 0) {
          ...
          if (nextTpIdx === 0) trailingSL = effectiveEntryPrice  // BE
          else trailingSL = t.tpLadder[nextTpIdx - 1]
        }
        nextTpIdx++
        tpProgressed = true
        if (remaining <= 1e-9) {
    ...
    // 3) Early-exit on bar CLOSE (after SL/TP processed for this bar).
    //    Use bar close price as the trigger — represents what trader would see/act on.
    //    Exit by market at next bar's open... but to keep it simple and conservative
    //    for the "I would have exited at +$X" interpretation, we exit AT THE CLOSE
    //    of the trigger bar. (Slippage models the latency to actual fill.)
    if (earlyExitUsdThreshold != null && earlyExitUsdThreshold > 0 && !earlyExitFired) {
      const unrealizedPerUnit = isLong ? c.close - effectiveEntryPrice : effectiveEntryPrice - c.close
      const unrealizedUsd = unrealizedPerUnit * positionUnits * remaining
      if (unrealizedUsd >= earlyExitUsdThreshold) {
        fills.push({ time: c.time, price: c.close, frac: remaining, kind: 'EARLY' })
        remaining = 0
        earlyExitFired = true
        closeTime = c.time
        break
  ...
  // 4) Force-close any leftover at the final candle close (EOD / period end).
  if (remaining > 1e-9) {
...
interface SimResult {
  ...
  earlyExitProfitUsd: number
  monthly: Map<string, { pnl: number; equity: number; trades: number }>
  /** kind → count of fills (helps see how many trades got cut short by EARLY) */
  fillCounts: Record<FillKind, number>
}

function emptyFillCounts(): Record<FillKind, number> {
  return { TP1: 0, TP2: 0, TP3: 0, SL: 0, EOD: 0, EARLY: 0 }
}

interface SimInput {
  /** All raw signals across the universe, sorted by entryTime */
  signals: RawSignal[]
  /** m5 candles per symbol (full series, walker slices forward from entry) */
  m5BySymbol: Map<string, OHLCV[]>
  btc: BtcRegime
  earlyExitPctOfDeposit: number  // 0 = disabled (baseline)
  label: string
}

function simulate(input: SimInput): SimResult {
  const { signals, m5BySymbol, btc, earlyExitPctOfDeposit, label } = input
  ...
  interface Active {
    ...
    marginUsd: number
    effectiveEntryPrice: number
    pendingFills: TradeFill[]      // pre-computed fills, drained in time order
    closeTime: number
  ...
  function realizeFillsUntil(t: number) {
    for (let ai = active.length - 1; ai >= 0; ai--) {
      ...
      while (a.pendingFills.length > 0 && a.pendingFills[0].time <= t) {
        ...
        // Fill kind → fee/slip model
        const isMaker = f.kind === 'TP1' || f.kind === 'TP2' || f.kind === 'TP3'
        let exitPrice: number
        if (isMaker) {
          exitPrice = f.price
        } else {
          // SL / EARLY / EOD = taker market → slip pushes worse
  ...
  for (const sig of sorted) {
    ...
    // Walk this trade through m5 to get fills (with optional early-exit)
    ...
    const earlyThresholdUsd = earlyExitPctOfDeposit > 0
      ? currentDeposit * earlyExitPctOfDeposit
      : null
    ...
    const { fills, closeTime } = walkTradeFills(
      simulated, effectiveEntry, sizing.positionUnits, earlyThresholdUsd,
    )
  ...
  // Anything still active gets force-closed (shouldn't happen with EOD safety in walker)
  ...
  return {
    ...
    earlyExitCount, earlyExitProfitUsd,
    monthly,
    fillCounts,
  }
}
...
function printResult(period: string, r: SimResult) {
  ...
  console.log(
    ...
    `SL=${r.fillCounts.SL} EOD=${r.fillCounts.EOD} EARLY=${r.fillCounts.EARLY}` +
    (r.earlyExitCount > 0 ? ` | early profit total=${fmtUsd(r.earlyExitProfitUsd)}` : '')
  )
...
async function main() {
  console.log('Daily Breakout — Variant B with EARLY-EXIT @ 2% of deposit (no $10 floor)')
  ...
  console.log(`Risk ${RISK_PCT}% | BTC ADX>${BTC_ADX_THRESHOLD} | dedup guards on | EOD-NO-TP1 (via 5m walker, no special EOD logic)`)
  console.log(`Early-exit threshold: deposit-at-open × ${(EARLY_EXIT_PCT_OF_DEPOSIT * 100).toFixed(1)}%`)
  ...
  // Load m5 once per symbol; we keep the full series so the walker can step forward
  ...
  // Generate signals once for each (symbol, period)
  ...
  function runBoth(period: string, sigs: RawSignal[]) {
    ...
    const baseline = simulate({
      ...
      earlyExitPctOfDeposit: 0,
      label: 'B baseline (no early-exit)',
    })
    const earlyExit = simulate({
      signals: sigs, m5BySymbol, btc,
      earlyExitPctOfDeposit: EARLY_EXIT_PCT_OF_DEPOSIT,
      label: `B early-exit @ ${(EARLY_EXIT_PCT_OF_DEPOSIT * 100).toFixed(0)}% of deposit`,
    })
    printResult(period, baseline)
    console.log()
    printResult(period, earlyExit)
    console.log()
    return { baseline, earlyExit }
  }
  ...
  // Δ summary
  console.log('================== Δ baseline → early-exit ==================')
  function delta(label: string, b: SimResult, e: SimResult) {
    ...
    console.log(
      ...
      `early=${e.earlyExitCount} (${e.trades > 0 ? ((e.earlyExitCount / e.trades) * 100).toFixed(0) : 0}% of trades)`
    )
  }
  delta('FULL', full.baseline, full.earlyExit)
  delta('TRAIN', train.baseline, train.earlyExit)
  delta('TEST', test.baseline, test.earlyExit)
  console.log()

  // Min-deposit risk check on early-exit variant
  console.log('================== Risk check (early-exit variant) ==================')
  ...
  risk('FULL', full.earlyExit)
  risk('TRAIN', train.earlyExit)
  risk('TEST', test.earlyExit)
  console.log()

  printMonthly('FULL baseline', full.baseline)
  console.log()
  printMonthly('FULL early-exit', full.earlyExit)
  console.log()
