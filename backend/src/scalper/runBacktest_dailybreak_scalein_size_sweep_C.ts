...
const RISK_PCT = 2
// sweep по размеру scale-in относительно primary
const SCALE_IN_FRACS = [0.25, 0.5, 0.75, 1.0]
let SCALE_IN_FRAC = 0.5  // mutable — устанавливается per-iteration
...
async function main() {
  console.log('Daily Breakout — Variant C (limit entry): SIZE SWEEP scale-in UP (25/50/75/100% of primary)')
  ...
  // SIZE SWEEP: для каждого SCALE_IN_FRAC прогоняем все 3 периода и собираем результат в таблицу.
  // Pool сигналов одинаковый для всех size — только sizing в simulate меняется.
  interface SweepRow { frac: number; period: 'FULL' | 'TRAIN' | 'TEST'; baseline: SimResult; scaleIn: SimResult }
  const rows: SweepRow[] = []
  for (const frac of SCALE_IN_FRACS) {
    SCALE_IN_FRAC = frac
    console.log(`================== SCALE_IN_FRAC = ${(frac * 100).toFixed(0)}% ==================`)
    for (const [periodName, periodKey] of [['FULL', 'full'], ['TRAIN', 'train'], ['TEST', 'test']] as const) {
      ...
      rows.push({ frac, period: periodName, baseline: baselineRes, scaleIn: scaleInRes })
  ...
  // Сводная таблица — FULL период
  console.log('================== FULL period summary across sizes ==================')
  console.log('size | final$ | ret% | R/tr  | WR  | DD  | Δret  | scaleIn{trades, WR, net$}')
  for (const r of rows.filter(r => r.period === 'FULL')) {
    ...
    console.log(
      `${(r.frac * 100).toFixed(0).padStart(3)}% | $${r.scaleIn.finalDeposit.toFixed(0).padStart(5)} | ` +
