/**
 * Daily Breakout — Variant B: "EOD FLAT" rule.
 *
 * Rule: at 23:55 UTC every day, close 100% of any open position by market —
 *       regardless of whether TP1 was hit. Equivalent to "no carry-over to
 *       next day, ever."
 *
 * Compares against current PROD baseline (EOD-NO-TP1 — trades that hit TP1
 * survive past midnight; only pre-TP1 trades are force-closed at 23:55).
 *
 ...
 *   - Exit splits: 50/30/20 ladder with full trailing (TP1→BE, TP2→TP1)
 *
 * Run: cd backend && npx tsx src/scalper/runBacktest_dailybreak_B_eod_flat.ts
 */
...
// EOD cutoff: 23:55 UTC = 86_100_000 ms into the day
const EOD_CUTOFF_MS = 23 * 3600_000 + 55 * 60_000
...
function eodCutoffOfDay(utcDate: string): number {
...
// ============================================================================
// Per-trade walker — supports two EOD modes:
//   'noTp1':  PROD baseline. At 23:55 UTC, close only if TP1 NOT yet hit.
//             Trades with TP1 hit survive past midnight, ride trailing SL.
//   'flat':   Close ALL open positions at 23:55 UTC, no exceptions.
// ============================================================================

type EodMode = 'noTp1' | 'flat'
...
function walkTradeFills(
  ...
  // Precompute the EOD cutoff timestamp for the day this trade opened.
  // For 'flat' mode we use it always. For 'noTp1' mode it's the same cutoff
  // but we skip force-close when TP1 was hit before reaching it.
  const entryDayCutoff = eodCutoffOfDay(t.utcDate)

  const lastIdx = t.m5.length - 1
  let closeTime = t.m5[lastIdx].time

  // We track the "current EOD cutoff" — when a trade survives past midnight,
  // the cutoff rolls forward to the next day at 23:55 UTC. (Only relevant
  // for 'noTp1' mode, where TP1+ trades can live multiple days.)
  let currentEodCutoff = entryDayCutoff

  for (let i = t.startIdx + 1; i <= lastIdx; i++) {
    const c = t.m5[i]

    // 0) EOD force-close check on bar TIME (close the trade at this bar's
    //    close price if bar covers/passes the cutoff).
    //    A 5m bar at time=T covers [T, T+5min]. The cutoff lives inside this
    //    bar if cutoff is in (T - 5min, T]. We use bar c.time as decision
    //    point: if c.time >= cutoff and bar opened before/at cutoff, close.
    //    Simpler heuristic: as soon as c.time >= currentEodCutoff, close.
    if (c.time >= currentEodCutoff) {
      const shouldFlatten = eodMode === 'flat' ? true : !tp1Hit
      if (shouldFlatten) {
        // EOD market close at this bar's close price.
        fills.push({ time: c.time, price: c.close, frac: remaining, kind: 'EOD' })
        remaining = 0
        closeTime = c.time
        break
      } else {
        // 'noTp1' mode and TP1 already hit — let it ride. Roll cutoff to next day.
        const nextDay = new Date(currentEodCutoff + 24 * 3600_000).toISOString().slice(0, 10)
        currentEodCutoff = eodCutoffOfDay(nextDay)
      }
    }
    ...
    while (tpProgressed && nextTpIdx < t.tpLadder.length && remaining > 1e-9) {
      ...
      if (fill) {
        ...
        if (actualFrac > 0) {
          ...
          if (nextTpIdx === 0) {
            trailingSL = effectiveEntryPrice  // BE
            tp1Hit = true
          } else {
            trailingSL = t.tpLadder[nextTpIdx - 1]
          }
        }
        nextTpIdx++
        tpProgressed = true
  ...
  // Force-close leftover at last available candle (period boundary)
  if (remaining > 1e-9) {
...
function simulate(input: SimInput): SimResult {
  const { signals, m5BySymbol, btc, eodMode, label } = input
    ...
    for (let ai = active.length - 1; ai >= 0; ai--) {
      ...
      while (a.pendingFills.length > 0 && a.pendingFills[0].time <= t) {
        ...
        if (isMaker) {
          ...
          // SL / EOD = taker market → slip pushes worse
  ...
  for (const sig of sorted) {
    ...
    const { fills, closeTime } = walkTradeFills(simulated, effectiveEntry, eodMode)
...
async function main() {
  console.log('Daily Breakout — Variant B: EOD-FLAT vs PROD baseline (EOD-NO-TP1)')
  ...
  console.log(`EOD cutoff: 23:55 UTC | Splits ${SPLITS.join('/')} with full trailing`)
  ...
  function runBoth(period: string, sigs: RawSignal[]) {
    ...
    const baseline = simulate({
      ...
      eodMode: 'noTp1',
      label: 'B baseline (EOD-NO-TP1: TP1+ trades survive)',
    })
    const flat = simulate({
      signals: sigs, m5BySymbol, btc,
      eodMode: 'flat',
      label: 'B EOD-FLAT (close ALL @ 23:55 UTC)',
    })
    printResult(period, baseline)
    console.log()
    printResult(period, flat)
    console.log()
  ...
  console.log('================== Δ EOD-NO-TP1 → EOD-FLAT ==================')
  function delta(label: string, b: SimResult, e: SimResult) {
    ...
    const eodChange = e.fillCounts.EOD - b.fillCounts.EOD
    console.log(
      ...
      `EOD fills ${b.fillCounts.EOD}→${e.fillCounts.EOD} (${eodChange >= 0 ? '+' : ''}${eodChange})`
    )
  }
  delta('FULL', full.baseline, full.flat)
  delta('TRAIN', train.baseline, train.flat)
  delta('TEST', test.baseline, test.flat)
  console.log()

  // Fill-distribution comparison: how many TP2/TP3 fills did we lose?
  console.log('================== Fill distribution ==================')
  function fillRow(label: string, r: SimResult) {
    const total = r.fillCounts.TP1 + r.fillCounts.TP2 + r.fillCounts.TP3 + r.fillCounts.SL + r.fillCounts.EOD
    const pct = (n: number) => total > 0 ? ((n / total) * 100).toFixed(0) : '0'
    console.log(
      `${label.padEnd(50)} | ` +
      `TP1 ${r.fillCounts.TP1}(${pct(r.fillCounts.TP1)}%) ` +
      `TP2 ${r.fillCounts.TP2}(${pct(r.fillCounts.TP2)}%) ` +
      `TP3 ${r.fillCounts.TP3}(${pct(r.fillCounts.TP3)}%) ` +
      `SL ${r.fillCounts.SL}(${pct(r.fillCounts.SL)}%) ` +
      `EOD ${r.fillCounts.EOD}(${pct(r.fillCounts.EOD)}%)`
    )
  }
  for (const [period, group] of [['FULL', full], ['TRAIN', train], ['TEST', test]] as const) {
    fillRow(`${period} | baseline (NO-TP1)`, group.baseline)
    fillRow(`${period} | flat`, group.flat)
  }
  console.log()

  console.log('================== Risk check (EOD-FLAT variant) ==================')
  ...
  risk('FULL', full.flat)
  risk('TRAIN', train.flat)
  risk('TEST', test.flat)
  console.log()

  printMonthly('FULL baseline (NO-TP1)', full.baseline)
  console.log()
  printMonthly('FULL EOD-FLAT', full.flat)
  console.log()
