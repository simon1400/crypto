/**
 * 365-day backtest of V2 levels strategy on additional forex pairs:
 * USDJPY, AUDUSD, USDCAD.
 *
 * Uses same loosened FOREX FiboCfg as production levelsLiveScanner.ts.
 *
 * Run: cd backend && npx tsx src/scalper/runBacktest365_forex.ts
 */
...
const SETUPS: SetupConfig[] = [
  { symbol: 'USDJPY', sides: ['BUY', 'SELL', 'BOTH'] },
  { symbol: 'AUDUSD', sides: ['BUY', 'SELL', 'BOTH'] },
  { symbol: 'USDCAD', sides: ['BUY', 'SELL', 'BOTH'] },
]

// FOREX-loosened config (matches production buildCfg for FOREX)
...
async function loadAll(setup: SetupConfig): Promise<LoadedData | null> {
  try {
    const m5  = await loadForexHistorical(setup.symbol, '5m',  MONTHS_BACK)
    const m15 = await loadForexHistorical(setup.symbol, '15m', MONTHS_BACK)
    const h1  = await loadForexHistorical(setup.symbol, '1h',  MONTHS_BACK)
    const d1  = await loadForexHistorical(setup.symbol, '1d',  MONTHS_BACK)
...
function recommendation(results: ComboResult[]) {
  console.log('\n=== FOREX RECOMMENDATION (R/trade > 0.3 AND trades >= 25) ===')
  ...
  if (passes.length === 0) console.log('No combo passes threshold.')
    ...
    for (const r of passes.sort((a, b) => b.rPerTrade - a.rPerTrade)) {
      console.log(`  ${r.symbol.padEnd(10)} ${r.side.padEnd(6)} → N=${r.trades}  R/trade=+${r.rPerTrade.toFixed(2)}  totalR=+${r.totalR.toFixed(1)}  PF=${fmtPF(r.profitFactor)}  WR=${r.winRate.toFixed(0)}%`)
    }
  }
  console.log('\nNear-miss:')
  ...
  for (const r of near.sort((a, b) => b.rPerTrade - a.rPerTrade)) {
    console.log(`  ${r.symbol.padEnd(10)} ${r.side.padEnd(6)} → N=${r.trades}  R/trade=+${r.rPerTrade.toFixed(2)}  totalR=+${r.totalR.toFixed(1)}`)
  }
}

async function main() {
  console.log(`Forex backtest: ${SETUPS.length} symbols × 3 sides`)
  console.log(`TWELVE_DATA_API_KEY: ${process.env.TWELVE_DATA_API_KEY ? 'present' : 'MISSING'}`)
  console.log('Sleeping 65s to clear any previous TwelveData rate-limit window...')
  ...
  for (const setup of SETUPS) {
    ...
    if (!data) {
      console.warn(`[${setup.symbol}] SKIPPED — sleeping 90s before next symbol to recover quota`)
      await new Promise((r) => setTimeout(r, 90_000))
      continue
    }
    // Pause between symbols to keep quota clean
  ...
  const outFile = path.join(outDir, `backtest365_forex_${Date.now()}.json`)
