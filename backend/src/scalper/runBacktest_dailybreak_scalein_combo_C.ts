...
const RISK_PCT = 2
// Combo sweep: (trigger, size) пары для C
const COMBOS = [
  { trigger: 0.33, size: 0.50, label: '33%/50%' },
  { trigger: 0.33, size: 0.75, label: '33%/75%' },
  { trigger: 0.33, size: 1.00, label: '33%/100%' },
  { trigger: 0.50, size: 0.50, label: '50%/50% (ref)' },
  { trigger: 0.50, size: 1.00, label: '50%/100%' },
]
let SCALE_IN_FRAC = 0.5  // mutable per combo
let TRIGGER_PCT = 0.5  // mutable per combo
...
async function main() {
  console.log('Daily Breakout — Variant C (limit entry): COMBO sweep (trigger × size)')
  ...
  interface ComboRow { label: string; trigger: number; size: number; period: 'FULL' | 'TRAIN' | 'TEST'; baseline: SimResult; scaleIn: SimResult }
  const rows: ComboRow[] = []
  for (const combo of COMBOS) {
    TRIGGER_PCT = combo.trigger
    SCALE_IN_FRAC = combo.size
    // Пересчитать scale-in pool с новым trigger (size меняется в simulate через SCALE_IN_FRAC)
    ...
    console.log(`================== COMBO ${combo.label} (trigger=${(combo.trigger * 100).toFixed(0)}%, size=${(combo.size * 100).toFixed(0)}%) ==================`)
    console.log(`scaleIn pool: FULL ${scaleInPools.full.length} | TRAIN ${scaleInPools.train.length} | TEST ${scaleInPools.test.length}`)
    for (const [periodName, periodKey] of [['FULL', 'full'], ['TRAIN', 'train'], ['TEST', 'test']] as const) {
      ...
      rows.push({ label: combo.label, trigger: combo.trigger, size: combo.size, period: periodName, baseline: baselineRes, scaleIn: scaleInRes })
      ...
      console.log(
        ...
        `scaleIn $${scaleInRes.finalDeposit.toFixed(0).padStart(7)} (${retS >= 0 ? '+' : ''}${retS.toFixed(0)}%) | ` +
  ...
  console.log('================== Combo summary by period ==================')
  for (const period of ['FULL', 'TRAIN', 'TEST'] as const) {
    console.log(`--- ${period} ---`)
    console.log('combo         | final$     | ret%   | R/tr  | WR  | DD  | Δret    | scaleIn{trades, WR, net$}')
    for (const r of rows.filter(r => r.period === period)) {
      const retS = ((r.scaleIn.finalDeposit / r.scaleIn.startingDeposit - 1) * 100)
      const retB = ((r.baseline.finalDeposit / r.baseline.startingDeposit - 1) * 100)
      const swr = r.scaleIn.scaleInCount > 0 ? (r.scaleIn.scaleInWins / r.scaleIn.scaleInCount) * 100 : 0
      console.log(
        `${r.label.padEnd(13)} | $${r.scaleIn.finalDeposit.toFixed(0).padStart(8)} | ` +
        `${(retS >= 0 ? '+' : '')}${retS.toFixed(0).padStart(5)}% | ${fmtR(r.scaleIn.rPerTr)} | ` +
        `${r.scaleIn.winRate.toFixed(0)}% | ${r.scaleIn.maxDD.toFixed(0)}% | ` +
        `${(retS - retB) >= 0 ? '+' : ''}${(retS - retB).toFixed(0).padStart(5)}pp | ` +
        `{${r.scaleIn.scaleInCount}, ${swr.toFixed(0)}%, ${fmtUsd(r.scaleIn.scaleInNet)}}`
      )
    }
    console.log()
  }
