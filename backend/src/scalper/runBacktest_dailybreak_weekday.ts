/**
 * Daily Breakout — weekday breakdown.
 *
 * Вопрос: есть ли edge на выходных (Sat/Sun) или они хуже будней?
 * Текущий prod: фильтра по weekday нет, стратегия торгует 7 дней в неделю.
 *
 * Группируем все сделки по UTC weekday (по entryTime) и считаем aggregate stats.
 * Анализ trade-уровневый (R/tr, WR), без deposit-симуляции — depo simulator по
 * группам не имеет смысла (теряется межгрупповой capital flow).
 *
 * Run: cd backend && npx tsx src/scalper/runBacktest_dailybreak_weekday.ts
 */
...
function aggregateByWeekday(pool: PortfolioTrade[]): WeekdayStats[] {
  const buckets: WeekdayStats[] = WEEKDAY_NAMES.map((n, i) => ({
    day: i, name: n, trades: 0, wins: 0, totalR: 0, rPerTr: 0, winRate: 0,
    isWeekend: i === 0 || i === 6,
  }))
  for (const t of pool) {
    const d = new Date(t.entryTime).getUTCDay()
    buckets[d].trades++
    buckets[d].totalR += t.pnlR
    if (t.pnlR > 0) buckets[d].wins++
  }
  for (const b of buckets) {
    b.rPerTr = b.trades > 0 ? b.totalR / b.trades : 0
...
async function main() {
  console.log('Daily Breakout — weekday breakdown (does the strategy work on weekends?)')
  console.log(`Symbols: ${PROD_SYMBOLS.length} | 365d history | Risk model: trade-level R (no deposit sim)`)
  console.log()
  ...
  for (const [label, pool] of [['FULL', allFull], ['TRAIN', allTrain], ['TEST', allTest]] as const) {
    const stats = aggregateByWeekday(pool)
    console.log(`=== ${label} ===`)
    console.log('Day  | trades |  wins | totalR  | R/tr  |  WR  ')
    console.log('-'.repeat(56))
    for (const s of stats) {
      const tag = s.isWeekend ? '*' : ' '
      console.log(`${s.name}${tag} | ${s.trades.toString().padStart(6)} | ${s.wins.toString().padStart(5)} | ${fmtR(s.totalR).padStart(7)} | ${fmtR(s.rPerTr).padStart(5)} | ${s.winRate.toFixed(0).padStart(3)}%`)
    }
    // Aggregate weekend vs weekday
    const weekend = stats.filter(s => s.isWeekend)
    const weekday = stats.filter(s => !s.isWeekend)
    const weTrades = weekend.reduce((a, s) => a + s.trades, 0)
    const weR = weekend.reduce((a, s) => a + s.totalR, 0)
    const weWins = weekend.reduce((a, s) => a + s.wins, 0)
    const wdTrades = weekday.reduce((a, s) => a + s.trades, 0)
    const wdR = weekday.reduce((a, s) => a + s.totalR, 0)
    const wdWins = weekday.reduce((a, s) => a + s.wins, 0)
    console.log('-'.repeat(56))
    console.log(`Weekend (Sat+Sun): ${weTrades} trades | totalR ${fmtR(weR)} | R/tr ${fmtR(weTrades > 0 ? weR / weTrades : 0)} | WR ${weTrades > 0 ? Math.round(100 * weWins / weTrades) : 0}%`)
    console.log(`Weekday (Mon-Fri): ${wdTrades} trades | totalR ${fmtR(wdR)} | R/tr ${fmtR(wdTrades > 0 ? wdR / wdTrades : 0)} | WR ${wdTrades > 0 ? Math.round(100 * wdWins / wdTrades) : 0}%`)
    console.log()
  }
