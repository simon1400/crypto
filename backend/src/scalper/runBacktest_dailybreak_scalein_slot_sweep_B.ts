/**
 * Daily Breakout — Variant B: SLOT POLICY sweep для scale-in UP.
 *
 * Сравниваем 3 политики concurrent slot'ов:
 *   shared      — scale-in занимает слот (текущий backtest). 20 conc total.
 *   freeScaleIn — scale-in НЕ считается в conc cap. Primary до 20, scale-in unlimited.
 *   doubled     — primary до 20 + scale-in до 20 отдельно (до 40 total).
 *
 * Параметры B: $320 / 20 conc / 5% margin / 2% risk / taker entry / 33%/75%.
 *
 * Run: cd backend && npx tsx src/scalper/runBacktest_dailybreak_scalein_slot_sweep_B.ts
 *
 * (заглушка legacy doc дальше — внутренняя логика идентична up_B.ts с заменой simulate)
 *
 * Логика scale-in UP:
...
const RISK_PCT = 2
const SCALE_IN_FRAC = 0.75  // B optimal — backtest 33%/75%

// Slot policy для scale-in:
//   'shared'      — scale-in занимает слот (default, backtest behaviour)
//   'freeScaleIn' — scale-in НЕ занимает слот (primary <=20, scale-in unlimited)
//   'doubled'     — primary <=20 AND scale-in <=20 (отдельные cap'ы, до 40 total)
...
function findScaleInTrigger(
  ...
  // B optimal trigger = 33% пути от entry к TP1 (из trigger sweep B)
  const midTP1 = entryPrice + 0.33 * (tp1 - entryPrice)
...
function simulate(allTrades: PortfolioTrade[], mode: Mode, btc: BtcRegime): SimResult {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    // Slot policy check — варьируется per iteration.
    //   shared:      active.length >= cap → skip (both primary and scale-in counted)
    //   freeScaleIn: только primary считается в cap; scale-in проходит всегда
    //   doubled:     primary <= cap AND scale-in <= cap (отдельные cap'ы)
    ...
    if (CURRENT_SLOT_POLICY === 'shared') {
      ...
      // scale-in всегда пропускаем; primary считается только в primary count
      if (!pt.isScaleIn) {
...
async function main() {
  console.log('Daily Breakout — Variant B: SLOT POLICY sweep (33%/75% scale-in)')
  ...
  // Baseline (без scale-in вообще) — одинаковый для всех slot policies
  CURRENT_SLOT_POLICY = 'shared'
  const baseFull = simulate(pools.full.primary, 'baseline', btc)
  const baseTrain = simulate(pools.train.primary, 'baseline', btc)
  const baseTest = simulate(pools.test.primary, 'baseline', btc)
  console.log(`Baseline (no scale-in) B: FULL $${baseFull.finalDeposit.toFixed(0)} (+${((baseFull.finalDeposit/baseFull.startingDeposit-1)*100).toFixed(0)}%)`)
  console.log()
  ...
  for (const policy of SLOT_POLICIES) {
    ...
    for (const [periodName, periodKey] of [['FULL', 'full'], ['TRAIN', 'train'], ['TEST', 'test']] as const) {
      const baselineRes = policy === 'shared' && periodKey === 'full' ? baseFull
        : policy === 'shared' && periodKey === 'train' ? baseTrain
        : policy === 'shared' && periodKey === 'test' ? baseTest
        : simulate(pools[periodKey].primary, 'baseline', btc)
      ...
      console.log(
        `${periodName.padEnd(6)} | baseline $${baselineRes.finalDeposit.toFixed(0).padStart(5)} (${retB >= 0 ? '+' : ''}${retB.toFixed(0)}%) | ` +
        `scaleIn $${scaleInRes.finalDeposit.toFixed(0).padStart(6)} (${retS >= 0 ? '+' : ''}${retS.toFixed(0)}%) | ` +
  ...
  for (const r of rows.filter(r => r.period === 'FULL')) {
    ...
    console.log(
      `${r.policy.padEnd(13)} | $${r.scaleIn.finalDeposit.toFixed(0).padStart(8)} | ${(retS >= 0 ? '+' : '')}${retS.toFixed(0).padStart(5)}% | ${fmtR(r.scaleIn.rPerTr)} | ${r.scaleIn.winRate.toFixed(0)}% | ${r.scaleIn.maxDD.toFixed(0)}% | ${r.scaleIn.opened.toString().padStart(5)} | ${r.scaleIn.scaleInCount.toString().padStart(5)} | ${swr.toFixed(0)}% | ${fmtUsd(r.scaleIn.scaleInNet)}`
    )
  }
  console.log()
