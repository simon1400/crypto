...
const RISK_PCT = 2
const SCALE_IN_FRAC = 0.5  // фиксированный 50% размер для trigger sweep
// sweep по trigger pct (доля пути от entry к TP1)
const TRIGGER_PCTS = [0.33, 0.5, 0.75]
let TRIGGER_PCT = 0.5  // mutable — устанавливается per-iteration
...
function findScaleInTrigger(
  ...
  // Trigger price = entry + TRIGGER_PCT * (tp1 - entry) для LONG, зеркально для SHORT
  ...
  for (let i = primaryEntryIdx + 1; i < m5.length; i++) {
    ...
    // trigger: бар ЗАКРЫЛСЯ за triggerPrice (TRIGGER_PCT-доля пути к TP1).
...
async function main() {
  console.log('Daily Breakout — Variant C (limit entry): TRIGGER SWEEP scale-in UP (33/50/75% path entry→TP1)')
  ...
  // Primary pool — одинаковый для всех TRIGGER_PCT (зависит только от cfg).
  // Кэшируем PrimarySignal'ы чтобы не пересчитывать для каждого trigger.
  ...
  interface SweepRow { trigger: number; period: 'FULL' | 'TRAIN' | 'TEST'; baseline: SimResult; scaleIn: SimResult; scaleInPoolSize: number }
  const rows: SweepRow[] = []
  for (const trig of TRIGGER_PCTS) {
    TRIGGER_PCT = trig
    // Пересчитать scale-in pool с новым trigger
    ...
    console.log(`================== TRIGGER_PCT = ${(trig * 100).toFixed(0)}% (path entry→TP1) ==================`)
    ...
    for (const [periodName, periodKey] of [['FULL', 'full'], ['TRAIN', 'train'], ['TEST', 'test']] as const) {
      ...
      rows.push({ trigger: trig, period: periodName, baseline: baselineRes, scaleIn: scaleInRes, scaleInPoolSize: scaleInPools[periodKey].length })
      ...
      console.log(
        ...
        `scaleIn $${scaleInRes.finalDeposit.toFixed(0).padStart(5)} (${retS >= 0 ? '+' : ''}${retS.toFixed(0)}%) | ` +
  ...
  console.log('================== FULL period summary across triggers ==================')
  console.log('trig | final$ | ret%  | R/tr  | WR  | DD  | Δret  | scaleIn{trades, WR, net$}')
  for (const r of rows.filter(r => r.period === 'FULL')) {
    ...
    console.log(
      `${(r.trigger * 100).toFixed(0).padStart(3)}% | $${r.scaleIn.finalDeposit.toFixed(0).padStart(5)} | ` +
      `${(retS >= 0 ? '+' : '')}${retS.toFixed(0).padStart(4)}% | ${fmtR(r.scaleIn.rPerTr)} | ` +
      `${r.scaleIn.winRate.toFixed(0)}% | ${r.scaleIn.maxDD.toFixed(0)}% | ` +
      `${(retS - retB) >= 0 ? '+' : ''}${(retS - retB).toFixed(0).padStart(4)}pp | ` +
