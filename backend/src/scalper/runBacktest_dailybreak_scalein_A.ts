/**
 * Daily Breakout — Variant A: scale-in (доливка) при движении в минус.
 *
 * Логика scale-in:
 *   1. Первичный вход: entry=c.close (taker), SL=rangeEdge противоположной, размер
 *      под 2% risk (как обычно)
 *   2. Если после входа цена дошла до mid(entry, SL) — т.е. отошла в МИНУС на полпути
 *      к SL — открываем ВТОРУЮ позицию (scale-in) того же направления:
 *      - entry = mid(entry, SL) (цена на этом баре)
 *      - SL = тот же rangeEdge (исходный SL первичной)
 *      - размер = +50% от первичного size (общий риск растёт ~×1.5)
 *      - таз же TP ladder (50/30/20 splits)
 *   3. Обе сделки живут независимо: первичная использует full trailing
 *      (TP1→BE, TP2→TP1, TP3→TP2), scale-in тоже full trailing.
 *
 * Гипотеза: если цена откатилась к 50% SL но НЕ выбила SL и развернулась — мы
 * получим profit на двойном объёме. Если SL всё-таки сработал — теряем ~3% а не 2%.
 *
 * Сравнение:
 *   - baseline: одна сделка на сетап (как PROD)
 *   - scale-in: первичная + потенциальная scale-in (когда цена дошла до midSL)
 *
 * Variant A: $500 / 10 conc / 10% target margin / 2% risk per trade
 *
 * Run: cd backend && npx tsx src/scalper/runBacktest_dailybreak_scalein_A.ts
 */
...
/**
 * Scan forward from primary entry: find first bar (within candles array) where price touched
 * mid(entry, SL) — т.е. отошла в минус на 50% к SL. Return the candle/index, or null.
 *
 * Останавливаем поиск если до этого:
 *  - SL hit (низ свечи <= SL для BUY) — добор не нужен, сделка уже мертва
 *  - TP1 hit (high >= TP1) — добор уже не нужен, сделка пошла в плюс мимо триггера
 *  - End of m5 array
 */
function findScaleInTrigger(
  ...
  const midSL = (entryPrice + sl) / 2  // 50% way к SL (в минусе)
  // Look forward from primaryEntryIdx + 1 (entry bar already consumed)
  for (let i = primaryEntryIdx + 1; i < m5.length; i++) {
    const c = m5[i]
    // SL hit на этом баре до midSL touch — нет смысла добирать
    const slHit = isLong ? c.low <= sl : c.high >= sl
    if (slHit) return null
    // TP1 hit — уже не нужно
    const tp1Hit = isLong ? c.high >= tp1 : c.low <= tp1
    if (tp1Hit) return null
    // midSL touch — для LONG low <= midSL; для SHORT high >= midSL
    const midTouch = isLong ? c.low <= midSL : c.high >= midSL
    if (midTouch) {
      // Триггер: midSL точка достигнута. Entry для добора = midSL ровно (как если бы у нас был
      // pre-placed limit на этой цене).
      return { idx: i, price: midSL }
    }
  }
  return null
}
...
async function main() {
  console.log('Daily Breakout — Variant A: baseline vs scale-in (averaging down @ 50% to SL)')
  ...
  console.log()
  console.log('SCALE-IN LOGIC:')
  console.log('  - Trigger: цена опустилась на 50% пути от entry к SL (т.е. -0.5R в минусе)')
  console.log('  - Размер: +50% от первичного position size')
  console.log('  - Entry: mid(entry, SL) — limit fill на этой цене')
  console.log('  - SL: тот же rangeEdge что у первичной')
  console.log('  - TP ladder: тот же')
  console.log('  - Trailing: full (TP1→BE, TP2→TP1, TP3→TP2)')
  console.log('  - Если SL hit ДО mid touch → scale-in не выставляется')
  console.log('  - Если TP1 hit ДО mid touch → scale-in не выставляется (тренд пошёл)')
  console.log('  - Общий риск на сетап: ~3% (2% primary + 1% scale-in, т.к. entry ближе к SL)')
  console.log()
  ...
  console.log(`Baseline check (must be ≈$2188 from AB-script): final=$${validate.finalDeposit.toFixed(2)}`)
  console.log()
  ...
  for (const [periodName, periodKey] of [['FULL (365d)', 'full'], ['TRAIN (60%, ~219d)', 'train'], ['TEST (40%, ~146d)', 'test']] as const) {
    ...
    const scaleInRes = simulate(allTrades, 'scaleIn', btc)
