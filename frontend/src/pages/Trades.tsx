...
export default function Trades() {
  ...
  async function exportCSV() {
    ...
    try {
      ...
      const header = ['ID', 'Монета', 'Тип', 'Плечо', 'Источник', 'Модель входа', 'Цена входа', 'Тип ордера', 'Stop Loss', 'TP1 цена', 'TP1 %', 'TP2 цена', 'TP2 %', 'TP3 цена', 'TP3 %', 'Цена закрытия', 'Размер (маржа)', 'Размер (с плечом)', 'Реализовано P&L (gross)', 'Комиссии', 'Фандинг', 'P&L (net)', 'P&L %', 'Статус', 'Открыта', 'Закрыта', 'Длительность', 'Закрытие 1 цена', 'Закрытие 1 %', 'Закрытие 1 P&L', 'Закрытие 1 дата', 'Закрытие 1 SL?', 'Закрытие 2 цена', 'Закрытие 2 %', 'Закрытие 2 P&L', 'Закрытие 2 дата', 'Закрытие 2 SL?', 'Закрытие 3 цена', 'Закрытие 3 %', 'Закрытие 3 P&L', 'Закрытие 3 дата', 'Закрытие 3 SL?', 'Score', 'Заметки']
      ...
      const rows = allTrades.map(t => {
        ...
        const closeData = (i: number) => {
          const c = closes[i]
          return c ? [c.price, c.percent + '%', fmt2(c.pnl) + '$', c.closedAt, c.isSL ? 'Да' : 'Нет'] : ['', '', '', '', '']
        }
        return [
          ...
          cp ?? '', fmt2(t.amount), fmt2(t.amount * t.leverage),
          fmt2(t.realizedPnl), fmt2(t.fees || 0), fmt2(t.fundingPaid || 0),
          fmt2(netPnl), fmt2(pnlPct) + '%', t.status,
          t.openedAt, t.closedAt || '', dur,
